    ...
    <div class='max-w-7xl mx-auto px-4 py-12'>
      ...
      <div class='bg-white rounded-2xl shadow-sm p-4 mb-10 flex flex-wrap gap-4 items-center'>
        ...
        <div class='flex gap-2 flex-wrap'>
          <button v-for='f in priceFilters' :key='f.label' @click='activeFilter = f.label'
            :class='["px-4 py-2 rounded-full text-sm font-medium transition-all",
              activeFilter === f.label ? "bg-pink-700 text-white" : "bg-gray-100 text-gray-600 hover:bg-pink-50 hover:text-pink-700"]'>
            {{ f.label }}
          </button>
        </div>
      ...
      <div v-if='!loading && filteredServices.length === 0' class='text-center py-20'>
        ...
        <button @click='resetFilters' class='mt-4 text-pink-700 font-medium hover:underline'>
          Clear filters
        </button>
      </div>
    </div>
...
// Helper — service field name can be 'name' or 'title' depending on API
const sName = (s) => s.name || s.title || ''
const sDesc = (s) => s.description || ''
...
const filteredServices = computed(() => {
  let result = services.value.filter(s => s.is_active)

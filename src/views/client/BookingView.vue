<!-- src/views/client/BookingView.vue -->
<template>
  <div class="min-h-screen bg-pink-50 py-12 px-4">
    <div class="max-w-2xl mx-auto">
      ...
      <div class="text-center mb-10">
        ...
        <p class="text-gray-500 mt-2">Choose how you'd like to book</p>
      </div>

      <!-- ── TAB SWITCHER ─────────────────────────────── -->
      <div class="flex bg-white rounded-2xl p-1 mb-8 shadow-sm border border-pink-100">
        <button
      ...
      <div v-if="activeTab === 'guest'">
        ...
        <div v-if="guestSuccess" class="bg-white rounded-3xl shadow-sm p-10 text-center">
          ...
          <div class="bg-pink-50 border-2 border-pink-200 rounded-2xl p-6 mb-6 text-left">
            <p class="text-pink-800 font-semibold text-sm mb-3">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex items-center justify-between bg-white rounded-xl px-4 py-2.5">
                ...
                <span class="font-mono font-medium text-gray-900">
                  {{ guestResult.account.email }}
                </span>
              </div>
              <div class="flex items-center justify-between bg-white rounded-xl px-4 py-2.5">
                <span class="text-gray-500">Password</span>
                <span class="font-mono font-bold text-pink-700 text-lg">
                  {{ guestResult.account.password }}
                </span>
              </div>
            </div>
            <p class="text-pink-600 text-xs mt-3">
              ⚠️ Save this password! It was also sent to your email.
          ...
          <div class="flex gap-3 justify-center">
            ...
            >Login to My Account</RouterLink>
        ...
        <div v-else class="bg-white rounded-3xl shadow-sm p-8 space-y-6">
          ...
          <div v-if="guestError"
            class="bg-rose-50 border border-rose-200 text-rose-700 rounded-xl
                   px-4 py-3 text-sm"
          >
            <!-- If email already exists, show login suggestion -->
          ...
          <div>
            <h3 class="font-semibold text-gray-900 mb-4">
              <span class="bg-pink-700 text-white w-6 h-6 rounded-full inline-flex
                           items-center justify-center text-xs mr-2">1</span>
              Your Details
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="sm:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">
                  Email Address * <span class="text-gray-400 font-normal">(your password will be sent here)</span>
                </label>
          ...
          <div>
            <h3 class="font-semibold text-gray-900 mb-4">
              <span class="bg-pink-700 text-white w-6 h-6 rounded-full inline-flex
                           items-center justify-center text-xs mr-2">2</span>
              Select a Service
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                ...
                type="button"
                @click="selectService(s, 'guest')"
                :class="[
                  'p-4 rounded-xl border-2 text-left transition-all',
                  guestForm.service_id === s.id
          ...
          <div>
            <h3 class="font-semibold text-gray-900 mb-3">
              <span class="bg-pink-700 text-white w-6 h-6 rounded-full inline-flex
                           items-center justify-center text-xs mr-2">3</span>
              Choose Date
            </h3>
          ...
          <div>
            <h3 class="font-semibold text-gray-900 mb-3">
              <span class="bg-pink-700 text-white w-6 h-6 rounded-full inline-flex
                           items-center justify-center text-xs mr-2">4</span>
              Select Time
          ...
          <!-- Submit -->
          <button
            @click="handleGuestBooking"
            :disabled="guestLoading || !guestForm.service_id
                       || !guestForm.start_time || !guestForm.reservation_date
                       || !guestForm.name || !guestForm.email"
      ...
      <div v-if="activeTab === 'login'">
        ...
        <!-- Step A: Login form (shown when not logged in) -->
        <div v-if="!auth.isLoggedIn" class="bg-white rounded-3xl shadow-sm p-8">
          <p class="text-gray-500 text-sm text-center mb-6">
            Sign in to your account to book an appointment
          </p>
          ...
          <p class="text-center text-gray-400 text-sm mt-4">
            ...
            <button @click="activeTab = 'guest'"
              class="text-pink-700 font-medium hover:underline"
            >Book as guest instead</button>
          </p>
        </div>

        <!-- Step B: Logged in — show booking form -->
        <div v-else>

          <!-- Welcome back message -->
          <div class="bg-pink-50 rounded-2xl px-5 py-3 mb-6 flex items-center
                      justify-between border border-pink-100">
          ...
          <div v-if="authSuccess" class="bg-white rounded-3xl shadow-sm p-10 text-center">
            ...
            <p class="text-gray-500 mb-6">
              {{ authSuccessData.service }} on
              {{ authSuccessData.date }} at {{ authSuccessData.time }}
            </p>
            <div class="flex gap-3 justify-center">
          ...
          <div v-else class="bg-white rounded-3xl shadow-sm p-8 space-y-6">
            ...
            <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button v-for="s in services" :key="s.id"
                  type="button"
                  @click="selectService(s, 'auth')"
                  :class="[
                    'p-4 rounded-xl border-2 text-left transition-all',
                    authForm.service_id === s.id
                      ? 'border-pink-700 bg-pink-50'
                      : 'border-gray-100 hover:border-pink-300'
                  ]"
                >
            ...
            <div>
              ...
              <textarea v-model="authForm.notes" rows="2"
            ...
            <button
              @click="handleAuthBooking"
              :disabled="authLoading || !authForm.service_id
                         || !authForm.start_time || !authForm.reservation_date"
              class="w-full bg-pink-700 text-white py-4 rounded-xl font-semibold text-lg
                     hover:bg-pink-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
import { fakeServices, guestBookingAPI, reservationsAPI, authAPI } from '../../services/api'
...
const guestForm = ref({
  name: '', email: '', phone: '',
  service_id: null, reservation_date: '', start_time: '', notes: ''
})
...
async function handleAuthBooking() {
  ...
  try {
    ...
    authSuccessData.value = {
      service: authSelectedService.value.title,
